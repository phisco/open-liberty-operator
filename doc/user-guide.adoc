...
_Note_: Open Liberty Operator watches for the creation and deletion of the SSO secret as well as any updates to it. Adding or removing keys from Secret will be passed down to the application automatically. However, updating the value of an existing key in `Secret` is not propagated to the application. See link:++./troubleshooting.adoc#single-sign-on-sso++[troubleshooting] for solution and an explanation.
